...
public class CallService extends Service implements SensorEventListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (action != null) {
            if (action.equals(Constants.ACTION_FOREGROUND_SERVICE_START)) {
                Notification notification = NotificationHelper.buildForegroundServiceNotification(getApplicationContext(), getString(R.string.call_service_notification_text));
                startForeground(10, notification);

                SensorManager sensorManager = (SensorManager) getSystemService(SENSOR_SERVICE);
                if (sensorManager != null) {
                    Sensor proximity = sensorManager.getDefaultSensor(TYPE_PROXIMITY);
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        if (sensorEvent.sensor.getType() == TYPE_PROXIMITY) {
